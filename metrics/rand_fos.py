    ...
    def __call__(self, stats_qzx, **kwargs):
        ...
        with torch.no_grad():
            for i in range(n_batches):
                ...
                idcs = np.random.choice(len(means_qzx), self.batch_size, replace=True)
                sub_z = means_qzx[idcs]
                sub_z = sub_z[..., pairs_of_latents]

                ref_range = torch.arange(self.batch_size, device=means_qzx.device)
                idcs_a = torch.tile(ref_range, dims=(self.batch_size,))
                idcs_b = torch.repeat_interleave(ref_range, self.batch_size)         
