...
# Script to run the G-Commutative S-N-VAE dSprites notebook with different seeds using papermill
...
INPUT_NOTEBOOK="$SCRIPT_DIR/s_n_beta_vae g-commutative (dsprites).ipynb"
...
echo "G-Commutative S-N-VAE dSprites Notebook Runner with Papermill"
echo "============================================================="
...
print_status $BLUE "Starting G-Commutative notebook execution..."
echo "------------------------------------------------"

# Run notebook for each seed
for seed in "${SEEDS[@]}"; do
    output_filename="s_n_beta_vae g-commutative (dsprites)_seed_${seed}.ipynb"
    output_path="$OUTPUT_DIR/$output_filename"
    
    print_status $BLUE "Running G-Commutative training with seed $seed..."
    ...
    if papermill \
        ...
        print_status $GREEN "✓ Successfully completed G-Commutative training with seed $seed"
    else
        failed_runs+=($seed)
        print_status $RED "✗ Failed to run G-Commutative training with seed $seed"
    fi
...
echo
echo "=================================================="
echo "G-COMMUTATIVE EXECUTION SUMMARY"
echo "=================================================="
...
if [[ ${#failed_runs[@]} -eq 0 ]]; then
    print_status $GREEN "🎉 All G-Commutative runs completed successfully!"
    print_status $GREEN "Results saved in: $OUTPUT_DIR"
    exit 0
else
    print_status $YELLOW "⚠️  Some G-Commutative runs failed. Check the error messages above."
    exit 1
fi